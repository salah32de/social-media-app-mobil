...
public class MessageAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if (holder.getItemViewType() == SEND_MESSAGE) {
            ((SendMessageViewHolder) holder).bind(context, messageList.get(position));
        } else {
            ((ReceiveMessageViewHolder) holder).bind(context, user, messageList.get(position));
        }
    }
...
//            sendMessageTime.setText(message.getTimestamp());
    ...
    public static class ReceiveMessageViewHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(Context context, User user, Message message) {
            if (message.getType().equals(Message.TEXT_TYPE)) {
...
//            receivedMessageTime.setText(message.getTimestamp());
            if (!user.getPhotoProfile().isEmpty())
                Glide.with(context).load(user.getPhotoProfile()).placeholder(R.drawable.user_cicrle_duotone).into(receiverProfileImage);
